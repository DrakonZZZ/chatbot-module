...
io.on('connection', (socket) => {
  const { id: idExists, serviceAgent } = socket.handshake.query;
  let clientId;

  if (serviceAgent) {
    console.log('customer service agent has connected');

    const format = Object.keys(chatLog).map((id) => ({
      id,
      messages: chatLog[id],
    }));

    socket.emit('CHAT_HISTORY', format);

    socket.on('disconnect', () => {
      console.log('Agent has disconnected');
    });
  } else {
